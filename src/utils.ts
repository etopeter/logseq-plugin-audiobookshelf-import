...
export const loadBooks = async (
  ...
  const url = new URL(serverUrl + "/api/libraries/" + library + "/items?limit=" + limit + "&page=" + page + "&filter=" + filter); // ZmluaXNoZWQ= aW4tcHJvZ3Jlc3M= filter in-progress
...
export const loadLibraries = async (
  ...
  if (filter.length > 0) {
    ...
    return jsonRes.libraries.
      filter(i => filter.includes(i.name))
  }
...
export function abLog(functionName: string, message: string) {
  // @ts-ignore
  if (logseq.settings.debug.includes(functionName)) console.info(functionName + "> " + message)
}
...
{ // day, h, m and s
  ...
  var minutes  = Math.floor(seconds / (60));
      seconds -= minutes * (60);
  return ((0<days)?(days+" d "):"")+hours+"h "+minutes+"m "+seconds.toFixed()+"s";
}